...
export const saleInvoicesApi = {
  list: async (params?: SaleInvoiceListParams): Promise<PaginatedData<SaleInvoice>> => {
    const response = await client.get<PaginatedData<SaleInvoice>>('/api/sale-invoices', { params });
    return response.data;
  },

  getPendingCredit: async (params?: PaginationParams): Promise<PaginatedData<SaleInvoice>> => {
    const response = await client.get<PaginatedData<SaleInvoice>>('/api/sale-invoices/pending-credit', { params });
    return response.data;
  },

  get: async (id: number): Promise<SaleInvoice> => {
    const response = await client.get<SaleInvoice>(`/api/sale-invoices/${id}`);
    return response.data;
  },

  create: async (body: CreateSaleInvoiceRequest): Promise<SaleInvoice> => {
    const response = await client.post<SaleInvoice>('/api/sale-invoices', body);
    return response.data;
  },
  ...
  delete: async (id: number): Promise<void> => {
    await client.delete(`/api/sale-invoices/${id}`);
  },
};
